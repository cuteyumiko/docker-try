...
import axios from 'axios'
  import hdpFooter from '../components/component-pc/footer.vue'
  import hdpTop from '../components/component-pc/top.vue'
  import hdpSearchAll from '../components/component-pc/searchall.vue'
  import hdpNav from '../components/component-pc/nav.vue'
  import hdpFooterSub from '../components/component-pc/footersub.vue'
  import Vue from 'vue'
  import { api_get_no, api_post } from '../assets/js/utils/api.js'
  import { url_stringfy } from '../assets/js/utils/utils.js'
  import * as HOST from '../assets/js/config_server.js'
  export default{
    async asyncData ({ params,query }) {
      // try{
        let {data:PicTextDetail} = await api_get_no({url:HOST.HOST_HDPCOMMODITY + '/commodityFront/getCommodityPicDetail',data:{commodityId: query.commodityId}})
        // let {data:PicTextDetail} = await axios.get(HOST.HOST_HDPCOMMODITY + '/commodityFront/getCommodityPicDetail',{params:{commodityId: query.commodityId}})
        // let {data} = await axios.get('http://hdpcommodity-api.99114.cn/commodityFront/getCommodityPicDetail?commodityId=79155220')
        let {data:recommendGoods} = await axios.get(HOST.HOST_HDPCOMMODITY + '/commodityFront/getCommodityDetailRecommendList',{params:{
          currNum: 1, pageSize: 6, version: 'v1.3', type: 0, commodityId: query.commodityId
        }})
        recommendGoods.respBody.forEach(e=>{
          e.pic = e.pic.split(',')[0]
        })
        return {
          PicTextDetail: PicTextDetail.detail,
          recommendGoods: recommendGoods.respBody,
          firstRecommendGoods:recommendGoods.respBody.slice(0,3),
        }
      // } catch(e){
      //   console.log(e)
      // }
...
//      },
        ...
        if (commodityId!=''){
          axios.get(url,{params:{
            ...
            if (response.status == 200) {
              _this.goodDetail = response.data
              _this.pcSalesPrice=_this.goodDetail.salesPrice
              if(_this.goodDetail.salesPrice.substring(0, _this.goodDetail.salesPrice.lastIndexOf('-') )!=''){
                _this.priceUP=_this.goodDetail.salesPrice.substring(0, _this.goodDetail.salesPrice.lastIndexOf('-') )
                _this.pricebottom=_this.goodDetail.salesPrice.substring(_this.goodDetail.salesPrice.lastIndexOf('-')+1, _this.goodDetail.salesPrice.length+1)
                let up=parseFloat(_this.priceUP)+parseFloat(_this.priceUP/100*50)
                let bottom=parseFloat(_this.pricebottom)+parseFloat(_this.pricebottom/100*50)
                  _this.MarketPrice=up.toFixed(2)+'-'+bottom.toFixed(2)

              }else{
                  let commodityId=_this.$route.query.commodityId
                  commodityId=_this.$route.query.commodityId
                ...
                if(_this.fixedArrey.indexOf(commodityId)!=-1&&obj[commodityId]){
                  _this.MarketPrice=obj[commodityId]
                }
                ...
                // }
                  ...
                  let MarketPrice =parseFloat(_this.goodDetail.salesPrice)+parseFloat(_this.goodDetail.salesPrice/100*50)
                  _this.MarketPrice=MarketPrice.toFixed(2)
                }

              }


              if (response.data.pic.split(',').length>4){
                _this.pics = response.data.pic.split(',').slice(0,4)
              }else{
                _this.pics = response.data.pic.split(',')
              }
              _this.currentImg=_this.pics[0]

              if (JSON.parse(_this.goodDetail.fullSingleActivity)!=null){
                _this.promotionActivity = JSON.parse(_this.goodDetail.fullSingleActivity)
                _this.promotionName=_this.promotionActivity[0].promotionName
              }

              let priceInterval = JSON.parse(response.data.standard).priceInterval
              _this.guige = JSON.parse( JSON.parse(priceInterval)[0].data );
              _this.storeNum= JSON.parse(response.data.standard).storeNum;
              for(let i=0;i<_this.guige.length;i++){
                if (_this.guige[i].Standard==null||_this.guige[i].Standard==''){
                  _this.showGuiGe=false
                }else{
                  _this.showGuiGe=true
                }
              }
      ...
      getParameters() {
        ...
        axios.get(url,{params:{commodityId: commodityId}}).then((response) => {
          if (response.status == 200) {
            this.PicTextDetail = response.data.detail
          }
        })
        // return axios.get(HOST_HDPCOMMODITY + '/commodityFront/getCommodityPicDetail',{commodityId: this.$route.query.commodityId})
        // .then((res)=>{
        //   return {PicTextDetail: response.body.detail}
        // })
      ...
      getRecommendGood(){
        ...
        if (commodityId){
          axios.get(url,{params:{currNum: 1, pageSize: 6, version: 'v1.3', type: 0, commodityId: commodityId}}).then((response) => {
            if (response.data.code == 200) {
              this.recommendGoods = response.data.respBody
...
//获取有图评价
      getImgComment:function(){
        ...
        axios.get(url,{params:{externalId: commodityId, start: this.pages, size: 5,onlyPicComment:1}}).then((response) => {
          if (response.status == 200) {
            this.imgComments = response.data.list
...
//      获取产品参数
      getParameters2:function(){
        ...
        axios.get(url,{params:{commodityId: commodityId}}).then((response) => {
          if (response.status == 200) {
            if (JSON.parse(response.data.attriDetail) == null) {
              this.proParameters = []
              this.noPicParams = false
            } else {
              let attriDetail = JSON.parse(response.data.attriDetail)
              this.noPicParams = true
